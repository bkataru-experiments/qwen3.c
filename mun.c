...
void malloc_run_state(RunState *s, Config *p) {
    ...
    if (!s->x || !s->xb || !s->xb2 || !s->xb3 || !s->hb || !s->hb2 || !s->q ||
...
void memory_map_weights(TransformerWeights *w, Config *p, void *pt) {
    ...
    ptr += p->vocab_size * p->dim;
    w->wk = ptr;
    ptr += p->dim * (p->n_kv_heads *
                     p->head_dim); // 1024 x 1024 = dim (1024) x num_kv_heads
                                   // (8) x p->head_dim (128)
    w->wk_norm = ptr;
    ...
    w->wo = ptr;
    ptr += (p->n_heads * p->head_dim) *
           p->dim; // attention heads (16) x head dim (128) * dim
    w->wq = ptr;
...
void read_checkpoint(char *checkpoint, Config *config,
    ...
    *data = mmap(NULL, *file_size, PROT_READ, MAP_PRIVATE, *fd, 0);
    ...
    void *weights_ptr =
        ((char *)*data) + 5951648; // skip header bytes. header_size = 5951648
                                   // TODO gguf total header = file size - (last
                                   // tensor size + last offset)
